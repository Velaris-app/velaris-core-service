...
CREATE OR REPLACE VIEW stats_trend_diff_view AS
SELECT
    ROW_NUMBER() OVER (ORDER BY a.owner_id, a.created_at) AS id,
    a.owner_id,
    a.created_at AS date,
    SUM(a.purchase_price * a.quantity) AS total_value,
    SUM(a.purchase_price * a.quantity)
      - LAG(SUM(a.purchase_price * a.quantity)) OVER (PARTITION BY a.owner_id ORDER BY a.created_at) AS delta_value,
    CAST(
        CASE
            WHEN LAG(SUM(a.purchase_price * a.quantity)) OVER (PARTITION BY a.owner_id ORDER BY a.created_at) IS NULL
            THEN NULL
            ELSE ((SUM(a.purchase_price * a.quantity)
                   - LAG(SUM(a.purchase_price * a.quantity)) OVER (PARTITION BY a.owner_id ORDER BY a.created_at))
                  / LAG(SUM(a.purchase_price * a.quantity)) OVER (PARTITION BY a.owner_id ORDER BY a.created_at)) * 100
        END AS DOUBLE
    ) AS delta_percent
FROM assets a
GROUP BY a.owner_id, a.created_at
ORDER BY a.owner_id, a.created_at;
